#!/usr/bin/env python3
"""
studio2ped_standalone.py - Convert Pedigree Studio session JSON to PED/MPED files.

Standalone single-file script. No installation required - needs only Python 3.9+.

Usage:
    python studio2ped_standalone.py session.json [-o prefix] [-d dir]
"""
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="studio2ped",
        description="Convert Pedigree Studio session JSON to PED/MPED files.")
    parser.add_argument("input", help="Path to the input session .json file")
    parser.add_argument("-o", "--output-prefix", default=None, dest="output_prefix",
    ...
    try:
        result = convert_file(args.input, output_prefix=args.output_prefix, output_dir=args.output_dir)
        print(result.summary)
        for filepath, _ in result.files:
            print(f"  Written: {filepath}")
        return 0
    except FileNotFoundError:
        print(f"Error: file not found: {args.input}", file=sys.stderr); return 1
